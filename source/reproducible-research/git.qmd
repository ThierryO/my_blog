...
image: git_files/figure-html/fig-prepare-repo-1.svg
knitr: 
  opts_chunk: 
    echo: false
    warning: false
...
```{r}
#| label: fig-basic
#| fig.cap: "An example of the history of a basic workflow"
#| fig.height: 1.5
library(tidyverse)
data.frame(x = 1:10, y = "master") %>%
  mutate(
    xend = lead(.data$x) - 0.1, yend = .data$y
  ) %>%
...
```{r}
#| label: fig-branching
#| fig.cap: "An example of a history with a few feature branches"
#| fig.height: 3
dataset <- tribble(
...
#| fig.cap: "An example of a history after two pull requests"
#| fig.height: 3
dataset <- tribble(
  ~commit, ~parent, ~branch,
  1, NA, "master",
...
```{r}
#| label: fig-prepare-repo
#| fig.cap: "Flowchart for preparing a repository."
library(diagram)
par(mar = rep(0, 4))
openplotmat()
...
textdiamond(
  c(0.5, 0.9), radx = 0.1, rady = 0.1,  lab = "Repository\nexists?",
  box.col = question
)
textrect(c(0.85, 0.9), radx = 0.1, lab = "Create\nrepository\nonline")
textdiamond(
  c(0.5, 0.5), radx = 0.1, rady = 0.1, lab = "Local clone\nexists?",
  box.col = question
)
textrect(c(0.1, 0.5), radx = 0.1, lab = "Clone\nrepository")
textellipse(
  c(0.5, 0.15), radx = 0.1, lab = "Check out\nfeature branch",
  box.col = end
)
```
...
#| fig.cap: "Flowchart for changing to a feature branch."
par(mar = rep(0, 4))
openplotmat()
...
textdiamond(
  c(0.5, 0.9), radx = 0.1, rady = 0.1, lab = "Feature\nbranch\nexists?",
  box.col = question
)
...
textrect(
  c(0.85, 0.15), radx = 0.1, lab = "Create and\ncheck out\nfeature branch"
)
...
```{r}
#| label: fig-commit
#| fig.cap: "Flowchart for applying changes in a feature branch."
par(mar = rep(0, 4))
openplotmat()
...
textdiamond(
  c(0.1, 0.15), radx = 0.1, rady = 0.1, lab = "Done with\nfeature?",
  box.col = question
)
textdiamond(
  c(0.5, 0.5), radx = 0.1, rady = 0.1, lab = "End of\nthe day?",
  box.col = question
)
...
textellipse(
  c(0.85, 0.9), radx = 0.1,  lab = "Check out\nmaster branch", box.col = end
)
```
