...
image: levels_files/figure-html/fig-recommendation-1.svg
knitr: 
  opts_chunk: 
    echo: false
    fig.width: 6
...
When we analyse a mixed models, the question often arises whether a covariate should be used as a random effect or as a fixed effects.
Let's assume a simple design.
Two types of fertilizer are tested on a number of fields ($n_s$).
Each field is split in two and the fertilizers are assigned at random to these halves while making sure that each field has both treatments.
From a conceptual point of view, we are only interested in the effect of the fertilizers.
However, we need to take the potential effect of the field into account.
Hence we use `fertiliser` as a fixed effect and `field` as a random effect.
...
Even when with a large number of levels ($n_s = 1000$), there still is a reasonable amount of uncertainty around $\sigma^2$ since the 95% interval of $r$ ranges from `r r_interval(1000, 0.975)` to `r r_interval(1000, 0.025)`.
The full distribution for $r$ when $n_s = 1000$ is shown in the figure below.

```{r}
#| label: fig-rel-var-1000
#| fig-cap: "Distribution of the relative true variance
#| ($\\frac{\\sigma^2}{s^2}$) when $n_s = 1000$. Dashed line:
#| $\\frac{\\sigma^2}{s^2} = 1$ Dotted lines: 2.5% and 97.5% quantiles of
#|  $\\frac{\\sigma^2}{s^2}$. Dash-dotted line: median of
#|  $\\frac{\\sigma^2}{s^2}$."
n <- 1000
data.frame(
  rel_var = exp(seq(
...
At $n_s = 20$, the 95% interval of $r$ will range from `r r_interval(20, 0.975)` to `r r_interval(20, 0.025)`.

```{r}
#| label: fig-rel-var-20
#| fig-cap: "Distribution of the relative true variance
#| ($\\frac{\\sigma^2}{s^2}$) when $n_s = 20$. Dashed line:
#| $\\frac{\\sigma^2}{s^2} = 1$ Dotted lines: 2.5% and 97.5% quantiles of
#|  $\\frac{\\sigma^2}{s^2}$. Dash-dotted line: median of
#|  $\\frac{\\sigma^2}{s^2}$."
n <- 20
...
The 95% interval of $r$ ranges from `r r_interval(4, 0.975)` to `r r_interval(4, 0.025)`, so we can hardly do any inference on the estimated variance.

```{r}
#| label: fig-rel-var-4
#| fig-cap: "Distribution of the relative true variance
#| ($\\frac{\\sigma^2}{s^2}$) when $n_s = 4$. Dashed line:
#| $\\frac{\\sigma^2}{s^2} = 1$ Dotted lines: 2.5% and 97.5% quantiles of
#| $\\frac{\\sigma^2}{s^2}$. Dash-dotted line: median of
#| $\\frac{\\sigma^2}{s^2}$."
n <- 4
...
#| fig-cap: "Uncertainty on the random effect variance in function of the number
