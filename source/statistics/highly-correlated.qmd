...
```{r}
#| label: fig-id-span
#| fig.cap: "Density of the span (difference between min and max) for all groups
...
dataset_cleaned %>%
  mutate(Resid = resid(model_cleaned)) %>%
...
#| fig.cap: "Scatterplot of the random intercepts and random slopes after
...
#| fig.cap: "Illustration of the most extreme random slopes after centering and
#| scaling."
#| dev: svg
dataset_cleaned <- dataset_cleaned %>%
  mutate(
...
```{r}
#| label: fig-fit-log
#| fig.cap: "Predictions for the fixed effect of the quadratic model on
#| $\\log(X)$"
dataset_cleaned <- dataset_cleaned %>%
