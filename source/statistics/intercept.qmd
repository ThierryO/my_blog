...
image: intercept_files/figure-html/fig-indices-rescaled-1.svg
...
The data consists of `r n_year` of data.
The number of observations in each year is `r effort` times the number of the year.
So the first year has `r effort` observation while the last year has `r effort * n_year` observations.
...
#| The points represent the observed counts, the line the true latent variable
#| $\\lambda$."
...
We create two new variables: `start` is a continuous variable centred as the first year, `fstart` is a factor with the first year as reference level.
...
```{r}
#| label: fig-indices-first
#| echo: false
#| fig-cap: "Indices based on the trend parameters when the first year is used
#| as reference."
#| message: false
library(scales)
...
```{r}
#| label: fig-indices-last
#| message: false
#| fig-cap: "Indices based on the trend parameters when the last year is used as
#| reference."
...
```{r}
#| label: fig-indices-rescaled
#| echo: false
#| fig-cap: "Indices using both reference years but rescaled so that the first
#| year equals 100%."
bind_rows(m1_t_index, m1_f_index) %>%
...
```{r}
#| label: fig-indices-alt
#| echo: false
#| fig-cap: "Indices based on rescaled abundance."
bind_rows(
