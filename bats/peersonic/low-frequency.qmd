...
image: low-frequency_files/figure-html/truth-spec-1.png
fixed: true
---
...
Now I have a file [test.wav](test.wav) which has sounds ranging from `r sprintf("%.0fkHz", min(sound.freq.range) / 1e3)` up to `r sprintf("%.0fkHz", max(sound.freq.range) / 1e3)`.
...
Note that the test file did not contain frequencies above `r sprintf("%.0fkHz", max(sound.freq.range) / 1e3)`.
