...
The bat detector is a [Peersonic RPA2](../peersonic/index.html) with [Philips SHB9850NC headphones](http://www.philips.co.uk/c-p/SHB9850NC_00/wireless-noise-cancelling-headphones).
...
At home you upload the observations to [waarnemingen.be](https://www.waarnemingen.be),
[waarneming.nl](https://www.waarneming.nl) or [observation.org](https://www.observation.org/).
...
[waarnemingen.be](https://www.waarnemingen.be) has unfortunately no API to download our data.
We had to download every track manually as a KML file.
Then we parse those file to get the observations and the tracks.
...
repo <- repository()
read_vc("bats/analysis/transect/observation", root = repo) |>
  mutate(
...
read_vc("bats/analysis/transect/transect", root = repo) |>
  select(-"order") |>
...
Only the common pipistrelle (_Pipistrellus pipistrellus_) has currently enough observations for the analysis (@tbl-observation-species).
...
#| label: thumbnail
#| results: hide
junk <- mapshot(map,file = "transect/prediction.png")
```
