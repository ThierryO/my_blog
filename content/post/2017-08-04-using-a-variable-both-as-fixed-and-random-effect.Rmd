...
We create a new dummy dataset with a discrete variable. The response variable is a third order polynomial of the discrete variable.
...
Suppose we fit a simple linear model to the data. We know that this is not accurate because the real pattern is a third order polynomial. And let's add the variable also as a random effect.
...
```{r discrete-fit, fig.cap = "Fitted values (line) and observed values (points) from the lme4 model. Results for the INLA model are identical.", echo = FALSE, warning = FALSE}
dataset %>%
  mutate(
