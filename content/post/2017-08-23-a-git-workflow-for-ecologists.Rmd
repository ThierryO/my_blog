...
The resulting history will look like [fig 1](#basic).
...
<a id="basic"></a>
```{r basic, fig.cap = "Fig 1. An example of the history of a basic workflow", fig.height = 1.5}
data.frame(
  x = 1:10,
  y = "master"
) %>%
...
[Fig 2](#branching) illustrates how the history looks like when a few branches are created.

<a id="branching"></a>
```{r branching, fig.cap = "Fig 2. An example of a history with a few feature branches", fig.height=3}
dataset <- tribble(
...
[Fig 3](#pull-request) illustrates the resulting history.
This can be done locally using a `merge`, but it is safer to do it on-line via a `pull request`.

<a id="pull-request"></a>
```{r pull-request, fig.cap = "Fig 3. An example of a history after two pull requests", fig.height=3}
dataset <- tribble(
  ~commit, ~parent, ~branch,
  1, NA, "master",
  2, 1, "master",
...
This is illustrated in [fig 3](#pull-request).
...
[Fig 4](#prepare-repo) illustrates the steps you need when you want to start working on a project.
Once you have a local `clone` of the repository you can `check out` the required feature branch ([fig 5](#create-branch)).
The last flowchart handles working in a feature branch and merge it when finished ([fig 6](#commit)).
...
<a id="prepare-repo"></a>
```{r prepare-repo, fig.cap = "Fig 4. Flowchart for preparing a repository."}
par(mar = rep(0, 4))
openplotmat()
...
```{r create-branch, fig.cap = "Fig 5. Flowchart for changing to a feature branch."}
par(mar = rep(0, 4))
openplotmat()
...
<a id="commit"></a>
```{r commit, fig.cap = "Fig 6. Flowchart for applying changes in a feature branch."}
par(mar = rep(0, 4))
openplotmat()
