...
tags: ["point pattern", "density", "leaflet", "projection"]
...
Plotting the raw data helps to check errors in the data.

```{r read-data, fig.cap = "Raw data", dev = "png", fig.alt = "A plot with the blue x placed at the coordinates of every point."}
...
The next step is to convert the dataset in to a `SpatialPoints` object with WGS84 project and re-project it into Lambert72.
`sp::CRS()` defines the coordinate systems.
`sp::coordinates()<-` is an easy way to convert a `data.frame` into a `SpatialPointsDataFrame`, but without specifying a coordinate system.
Therefore we need to override the `proj4string` slot with the correct coordinate system.
`sp::spTransform()` converts the spatial object from the current coordinate system to another coordinate system.
...
In order to visualise the result, we have to re-project the coordinates back to WGS84.
Then we can display the raster with a web based background image.

```{r density, fig.cap = "Static image of density"}
dens %>%
  as.data.frame() %>%
  group_by(id) %>%
  slice(1) %>%
...
So we converted the `data.frame` in a `SpatialPolygonsDataframe`, which is a combination of a `SpatialPolygons` and a `data.frame`.
The `SpatialPolygons` consists of a list of `Polygons`, one for each row of the `data.frame`.
A `Polygons` object consist of a list of one or more `Polygon` object.
In this example a single polygon which represents the grid cell.
...
```{r leaflet, fig.cap = "Dynamic map of density", eval = interactive()}
library(leaflet)
pal <- colorNumeric(
...
```{r leaflet-0, echo = FALSE, comment = ""}
library(leaflet)
pal <- colorNumeric(
  palette = rev(rainbow(100, start = 0, end = .7)),
  domain = c(0, dens_sp$density)
