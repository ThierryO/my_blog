...
I've reviewed the [standard version](http://peersonic.co.uk) the Peersonic RPA2 bat-detector in an [earlier blog post](../../06/peersonic).
Today I will take about the RPA2 in a [waterproof IP67 enclosure](http://peersonic.co.uk/advanced/) and my experiences on using it as on autonomous bat-detector.
...
![Open RPA2 detector in IP67 version.](/images/peersonic/peersonic_ip67_open.jpg)
...
```{r mic, fig.cap = "External microphone mounted on a tripod.", eval = FALSE}
```
...
```{r closed, fig.cap = "Closed RPA2 detector in IP67 version.", eval = FALSE}
```
...
The next figure gives an indication of the length of the recordings at the forest setting were the pictures were taken.
...
The following figure shows the distribution of the time between two consecutive recordings.
